...
import { updateEventRequestClientAction } from "./actions";
...
export function EditRequestForm({
  ...
  async function onSubmit(values: EventRequestInput) {
    setServerError(null);
    const result = await updateEventRequestClientAction(companySlug, eventId, values);
    if (result?.error) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="gap-1.5">
          <Pencil className="size-3.5" /> Editar solicitud
        </Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <DialogHeader>
            ...
            <DialogDescription>Puedes editarla mientras siga pendiente de revisión.</DialogDescription>
          </DialogHeader>
