...
export async function GET(request: NextRequest) {
  ...
  const doc = new jsPDF();
  doc.setFontSize(14);
  doc.text("Reporte de pagos — Operix", 14, 16);
  doc.setFontSize(10);
  doc.text(`Periodo: ${periodStart ?? "-"} a ${periodEnd ?? "-"}`, 14, 22);

  autoTable(doc, {
    startY: 28,
    head: [["Trabajador", "Reg.", "Extra", "Dom.", "Fer.", "Bonos", "Desc.", "Total", "Estado"]],
    body: records.map((r) => [
      r.worker.user.name,
      Number(r.regularHours).toFixed(1),
      Number(r.overtimeHours).toFixed(1),
      Number(r.sundayHours).toFixed(1),
      Number(r.holidayHours).toFixed(1),
